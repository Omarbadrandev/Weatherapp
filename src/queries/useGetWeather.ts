 ...
 let lon: number, lat:number
   ...
   //in the store is an empty array which gives us that the coordinates exists even if they are empty
    ...
    const fetchWeather  = async () => {
      const {data: weather} =  
      await axios.get(openWeatherMapBaseURL+`data/2.5/weather?lat=${lat}&lon=${lon}&appid=8e51c0a8fb74fccdd63a052107469797`);
      return weather
    }
    ...
    const weatherForcast = useQuery<Weather,AxiosError>([WeatherKey, coordinates],fetchWeather, {
